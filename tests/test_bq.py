...
class BqTest(unittest.TestCase):
    def setUp(self):
        ...
        self.db_path = Path(self.temp.name) / "bq.db"
        self.env = patch.dict(os.environ, {"BQ_DB": str(self.db_path)})
        self.env.start()
    ...
    def configure(self, rate="5", initial=None):
        with patch("time.time", return_value=1000), patch("builtins.print"):
            bq.set_budget(argparse.Namespace(rate=rate, initial=initial))

    def add(self, command=None):
        command = command or ["python", "-c", "pass"]
        with patch("time.time", return_value=1000), patch("builtins.print"):
            bq.add(argparse.Namespace(cwd=self.temp.name, command=command))
        return 1
    ...
    def test_budget_accrues_hourly_and_subtracts_charges(self):
        ...
        task_id = self.add()
        with patch("time.time", return_value=1000), patch("builtins.print"):
            bq.charge(argparse.Namespace(task_id=task_id, amount="1.5"))
        with bq.connect() as db:
            current, credits, spent = bq.balance(db, now=1000 + 2 * 3600 + 10)
    ...
    def test_retry_requeues_failed_task(self):
        ...
        bq.retry(argparse.Namespace(task_id=1))
        with bq.connect() as db:
            self.assertEqual(bq.get_task(db, 1)["status"], "queued")
    ...
    def test_parser_preserves_command_arguments(self):
        ...
        with patch("builtins.print"):
            bq.add(args)
        with bq.connect() as db:
            argv = json.loads(bq.get_task(db, 1)["argv"])
