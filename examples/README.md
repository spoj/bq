...
Then enqueue `bq-pi` like any other command:

```bash
cd ~/src/app
bq add bq-pi -p "Fix the failing tests"
```

When the worker starts `bq-pi`, it runs Pi and charges the task for usage added to its sessions. It includes `pi-tiny-fork` children by following their transcript paths and deduplicating history copied from parent sessions.
